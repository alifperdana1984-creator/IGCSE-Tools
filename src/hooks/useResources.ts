...
export function useResources(user: User | null, notify: NotifyFn) {
  ...
  const uploadResource = useCallback(
    async (
      ...
      if (!user) {
        notify("Login required to save resources", "error");
        return null;
      }
      setUploading(true);
      try {
        ...
        setKnowledgeBase((kb) =>
          kb.find((x) => x.id === resource.id) ? kb : [resource, ...kb],
        );
  ...
  const updateResourceType = useCallback(
    async (resource: Resource, resourceType: ResourceType) => {
      try {
        await fbUpdateResourceType(resource.id, resourceType);
        setResources((r) =>
          r.map((x) => (x.id === resource.id ? { ...x, resourceType } : x)),
        );
        setKnowledgeBase((kb) =>
          kb.map((x) => (x.id === resource.id ? { ...x, resourceType } : x)),
        );
      } catch (e) {
  ...
  const updateGeminiUri = useCallback(
    async (resource: Resource, uri: string) => {
      try {
        await fbUpdateGeminiUri(resource.id, uri);
        setResources((r) =>
          r.map((x) =>
            x.id === resource.id ? { ...x, geminiFileUri: uri } : x,
          ),
        );
        setKnowledgeBase((kb) =>
          kb.map((x) =>
  ...
  const processSyllabus = useCallback(
    async (resource: Resource, apiKey: string): Promise<void> => {
      try {
        const existing = await getSyllabusCache(resource.id);
        if (existing) return;
      } catch {
        /* cache read failed — proceed with processing */
      }
...
Be thorough — include every numbered objective. Do not skip any.`,
        ...
        const topics = JSON.parse(response.text || "{}") as Record<
          string,
          string
        >;
        if (Object.keys(topics).length > 0) {
          ...
          notify(
            `Syllabus "${resource.name}" processed — objectives cached`,
            "success",
          );
        }
      } catch (e) {
  ...
  const toggleShared = useCallback(
    async (resource: Resource, isShared: boolean) => {
      try {
        await fbToggleShared(resource.id, isShared);
        setResources((r) =>
          r.map((x) => (x.id === resource.id ? { ...x, isShared } : x)),
        );
        setKnowledgeBase((kb) =>
          kb.map((x) => (x.id === resource.id ? { ...x, isShared } : x)),
        );
      } catch (e) {
  ...
  const processPastPaper = useCallback(
    async (resource: Resource, apiKey: string): Promise<void> => {
      try {
...
Extract as many items as possible — aim for complete coverage of the paper, not a sample.`,
        ...
        const items = (parsed.items ?? [])
          .filter(
            (x) =>
              (x.questionText ?? "").trim().length > 10 &&
              (x.markScheme ?? "").trim().length > 5,
          )
          .map((x) => ({
            questionText: (x.questionText ?? "").trim(),
            commandWord: (x.commandWord ?? "").trim() || "Unknown",
            marks: Number.isFinite(x.marks as number)
              ? Math.max(1, Math.round(Number(x.marks)))
              : 1,
            markScheme: (x.markScheme ?? "").trim(),
            ...(x.questionType
              ? { questionType: String(x.questionType).trim() }
              : {}),
  ...
  const queueSyllabus = useCallback(
    (resource: Resource, apiKey: string) => {
      enqueueProcessing(() => processSyllabus(resource, apiKey));
    },
    [enqueueProcessing, processSyllabus],
  );

  const queuePastPaper = useCallback(
    (resource: Resource, apiKey: string) => {
